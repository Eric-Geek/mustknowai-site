...
import Link from 'next/link'; // 使用 Next.js的Link组件进行导航
...
export default function LandingPage({ t, title, description }: InferGetStaticPropsType<typeof getStaticProps>) {
  ...
  const featuredTools = [
    {
      ...
      name: "ChatGPT Plus",
      pitch: "高级AI对话，处理复杂任务", // 示例，真实场景下这些也应该来自翻译文件
  ...
  const categories = [
    {
      ...
      titleKey: "categoryWriting", // 假设你在common.json中有 "categoryWriting": "写作"
      descriptionKey: "categoryWritingDesc", // "AI写作工具和内容生成"
  ...
  // 确保在 common.json 中为以上 titleKey 和 descriptionKey 添加翻译

  const articles = [ // 文章数据也应该考虑国际化，或者至少 slug 是唯一的
    {
      slug: "getting-started-with-ai-art-generation", // 用于生成链接
      ...
      <header className="sticky top-0 z-50 w-full border-b border-white/20 bg-white/80 backdrop-blur-md dark:bg-slate-950/80">
        <div className="container mx-auto flex h-16 items-center justify-between px-4">
          ...
          <nav className="hidden md:flex items-center space-x-6">
            ...
            {/* <Link href="/about" locale={locale} className="text-slate-700 hover:text-blue-600 dark:text-slate-300 dark:hover:text-blue-400 transition-colors">
                {t.navAbout}
            </Link> */}
            <LanguageSwitcher />
          </nav>
          ...
          <div className="md:hidden flex items-center">
            <LanguageSwitcher />
            <Button variant="ghost" size="icon" aria-label="Toggle menu"> {/* SEO: Add aria-label */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {categories.map((category, index) => (
              <Card
                ...
                onClick={() => router.push(`/tools?category=${category.query}`)}
              >
              ...
              >
                <Link href={`/blog/${article.slug}`} className="block">
                  ...
                  <CardContent className="p-6">
                    ...
                    <span // Changed from Button to span for non-interactive "Read more" text within Link
